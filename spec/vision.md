...
Multiple selected workflows run together and share their Binance controller. Shutdown or a listener exiting stops the whole group, allowing accepted messages to drain before trading resources close. Duplicate source/destination pairs are rejected, but notices received independently from different sources may still trigger separate trades. The shared heartbeat can be refreshed by either listener and does not establish that all configured channels are healthy.
