...
`start_trader` accepts a non-empty sequence of unique event/trade pairs through `combinations`. It creates one workflow per pair and shares a trade controller per destination within that runtime, starting and stopping each destination once. All trading controllers start before listeners run concurrently. A shutdown request or any listener exit stops the whole group, drains accepted messages, and then stops trading controllers. Partial startup, cancellation, and cleanup failures must still attempt cleanup of all constructed workflows. `start_trader` accepts only `stop_event` and `combinations`; omitted combinations default to Upbit/Binance. `run_workflows` manages already assembled workflows so the latency diagnostic can supply its profiling handler directly to `EventToTrade` without adding injection arguments to `start_trader`. Telegram listeners retain independent connections and queues; the shared heartbeat currently indicates that at least one listener is making progress, not that every configured channel is healthy.
