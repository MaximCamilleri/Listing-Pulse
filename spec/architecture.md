...
`main.py` is responsible for composing runtime workflows from `src/interface/`. It builds event/trade pairs for each non-blank Upbit or Bithumb channel setting and passes them to `start_trader`, after configuring logging, resetting health, and installing shutdown handlers. If neither source is configured, startup logs a configuration error and exits with status 1 before creating controllers.
...
The event-to-trade workflow uses `src/control/event_to_trade.py` as an explicit composition exception: `EventToTrade` accepts an event source (`UPBIT` or `BITHUMB`) and trade source (`BINANCE`), selects the notice parser, creates the trade controller, and binds the listener callback. `listener_factory` constructs an inactive Telegram controller without scheduling tasks. `src/interface/trade_interface.py` owns controller startup, listener tasks, shutdown, and cleanup. Both notice parsers return asset symbols for KRW listings. Construction must not start listeners or place trades.

`start_trader` accepts a non-empty sequence of unique event/trade pairs through `combinations`. It creates one workflow per pair and shares a trade controller per destination within that runtime, starting and stopping each destination once. All trading controllers start before listeners run concurrently. A shutdown request or a connection supervisor exit stops the whole group, drains accepted messages, and then stops trading controllers. Partial startup, cancellation, and cleanup failures must still attempt cleanup of all constructed workflows. `start_trader` accepts only `stop_event` and `combinations`; omitted combinations default to Upbit/Binance. `run_workflows` manages already assembled workflows so the latency diagnostic can supply its profiling handler directly to `EventToTrade` without adding injection arguments to `start_trader`. Telegram channel controllers share one explicitly injected `TelegramIntegration` per runtime and retain independent bounded queues and parsers. Each channel controller registers its subscription and starts its queue worker before the interface starts the shared connection supervisor once. Readiness requires activation of every registered subscription. The interface stops the shared transport before unsubscribing and draining channel queues, then closes trading controllers. Channel controllers never own connection startup or shutdown. The shared supervisor owns reconnects and the single health heartbeat; health remains a liveness signal with bounded startup recovery, not proof of message delivery.
